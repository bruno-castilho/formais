...
class GLC:
    ...
    def __init__(self, producoes):
        ...
        Parameters
        ----------
        producoes: dicionário com as produções geradas por cada não-terminal da gramática
        """
    ...
    def calculaConjuntoFirst(self):
        """Calcula o conjunto de 'First' para cada não-terminal da gramática"""

        def calculaFirst(valor):
            """
            Calcula o conjunto de 'First' para um não-terminal específico da gramática

            Parameters
            ----------
            valor: str não-terminal
            """
            if valor in self.T:
                return [valor]
    ...
    def calculaConjuntoFollow(self):
        ...
        def insere(n, index):
            ...
            ----------
            n: str não-terminal
            index: int índice do não terminal na produção
            """
    ...
    def constroiTabelaLL(self):
        ...
        for j in range(len(linhas)):
            linha = [linhas[j], elementos[0: len(linhas)]]
    ...
    def fatora(gramatica):
        ...
        Parameters
        ----------
        gramatica: GLC instância de uma gramática
        """
        ...
        def direto(producoes):
            ...
            Parameters
            ----------
            producoes: lista de produções de um não terminal
            """
            ...
            for elt, items in groupby(sorted(producoes), itemgetter(0)):
                ...
                if len(items) > 1:
                    ...
                    novasProducoes[(f'{nt}{contador}',)] = novasProducoes.get((f'{nt}{contador}',), []) + [
                        x[1:] or ['&'] for x in items]
                    contador += 1
                else:
        ...
        def indireto(producoes, visitados):
            ...
            ----------
            producoes: lista de produções de um não terminal
            visitados: lista de não terminais já visitados
            """

            while True:
                lr = [prod for prod in producoes if
                      prod[0] in gramatica.N and prod[0] not in visitados]  # Produções com ambiguidade indireta
                nr = [prod for prod in producoes if
                      prod[0] not in gramatica.N or prod[0] in visitados]  # Produções sem ambiguidade indireta
        ...
        while True:
            ...
            contador += 1
            if contador >= limite:
                raise Exception('Limite de execuções atingido, a gramática é inerentemente ambígua.')
    ...
    def eliminaRecursaoEsquerda(gramatica):
        ...
        def eliminaRecursao(producoes, limite=100, contador=0):
            ...
            ----------
            producoes: lista de produções de um não terminal
            limite [default=100]: int limite de execuções
            contador [default=0]: int contador de execuções
            """
            while True:
    ...
    def lerGramatica(arquivo):
        """
        Lê o arquivo e retorna a gramática
